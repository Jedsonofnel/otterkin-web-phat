...
func CreateArtwork(app core.App, c echo.Context) (Artwork, error) {
	artwork, err := app.Dao().FindCollectionByNameOrId("artwork")
	if err != nil {
		return Artwork{}, err
	}

	newArtwork := models.NewRecord(artwork)
	...
	return GetArtworkById(app.Dao(), artwork.Id)
}

func GetArtworkById(dao *daos.Dao, id string) (Artwork, error) {
	artwork := Artwork{}
	err := dao.DB().
		Select("artwork.id as artwork_id", "artwork.*", "artists.user_id").
		From("artwork").
		InnerJoin("artists", dbx.NewExp("artwork.artist_id = artists.id")).
...
func UpdateArtworkById(app core.App, c echo.Context, id string) (Artwork, error) {
	artwork, err := app.Dao().FindRecordById("artwork", id)
	if err != nil {
		return Artwork{}, err
	}
...
func DeleteArtworkById(dao *daos.Dao, id string) error {
	artwork, err := dao.FindRecordById("artwork", id)
	if err != nil {
		return err
	}
